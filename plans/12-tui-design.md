...
- 手动 compact 成功后，agent working memory 会立即变为 continuation state + recent messages。
...
- 自动 compact 的错误只在模型压缩和 deterministic fallback 后仍不能降到安全阈值时展示，并提示用户手动 `/compact`、减少输入或新开会话。
