...
| 5 | [05-capability.md](05-capability.md) | 行动层 | 通用 Agent Skill 兼容、对话式导入/check/启用，以及 MCP 配置接入 |
...
| 15 | [15-single-agent-multi-ui.md](15-single-agent-multi-ui.md) | Daemon/Protocol | 单 Agent、多 UI 协作、event subscription 和 persistent transport 生命周期设计 |
...
6. 精简交互 (vs 命令爆炸)
   ...
   - Skill 通过对话式导入/生成/check/启用，不引入复杂命令
   - 高级功能通过自然语言交互
...
| 3 | 记忆深化 + 学习：active memory 质量评估 + 通用 Skill 导入/生成/check/加载 + MCP 接入 | 4 周 |
