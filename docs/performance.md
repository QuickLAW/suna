...
- **长历史不全量重放或渲染**：模型上下文使用 Session State + recent window；TUI Chat transcript 使用业务层虚拟滚动，只渲染当前窗口和 overscan，不把完整历史交给 viewport。
...
- 输入预算按模型配置计算：`context_window - max_output_tokens - margin`，其中 `margin = max(2048, context_window / 200)`。Suna 不再使用 `context_window * 0.8` 作为压缩阈值。
...
streaming 文本只追加时，TUI 会复用当前消息已经 wrap 好的行，并只增量处理新 delta、换行状态和最后一行宽度；不会在每个 chunk 上重新 split/wrap/join 完整已生成回复。窗口宽度变化或内容回退时才完整重算。这个优化不降低刷新率、不丢弃 delta、不延迟 done/tool/guard 等关键事件，而是用少量当前 streaming 消息的渲染行缓存换掉长回复 O(n²) 重排 CPU。缓存规模与当前消息长度同阶，不随 chunk 数增长。

这个设计保留高帧率实时反馈，同时避免半截 Markdown 造成行数抖动、viewport 抖动和高频重排。最终 completed 内容仍是 Markdown，不降低最终阅读体验。
...
长历史 Chat transcript 使用窗口化渲染。这里的“虚拟滚动”指 Suna 在 Chat 业务层维护完整消息和全局滚动位置，但只把当前可见窗口交给 viewport；它不是自定义 terminal renderer，也不是依赖宿主终端原生 scrollback：

- 完整消息保留在 `pages/chat.Model.Messages`。
...
该设计参考 Bubbles `table` / `list` 的窗口化思路：完整数据留在业务 model，View 只渲染当前窗口，而不是把无限增长的历史交给 viewport。因此滚动成本主要跟终端高度、overscan 和当前窗口内容相关，不直接随完整 transcript 线性增长。连续滚动不会被延迟或丢弃；优化点是跳过未跨窗口滚动时的 transcript 重建。
