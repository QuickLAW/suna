...
| 层 | 核心创新 | vs 现有 Claw |
|---|---|---|
| **输入/感知层 Input/Sense** | 当前处理 TUI/IPC 输入；Timer/Watcher/Webhook/Stream 为预留设计 | 传统 agent 是被动唤醒 |
...
| # | 文档 | 对应层次 | 说明 |
|---|---|---|---|
| 1 | [01-architecture.md](01-architecture.md) | 全局 | Daemon 架构、Agent/Runner/Subtask、IPC 通信 |
...
3. 主动感知 (vs 纯被动，目标设计)
   ...
   - 感知信号直接驱动行动层
   - 当前只保留 triggers 表和 IPC 常量预留，runtime 未接入
...
| Phase | 内容 | 周期 |
|---|---|---|
| 1 | Daemon 基础 + 记忆层基础：sunad + IPC + active memory + memory_queue + 9 tool definitions + Guard stub | 5 周 |
