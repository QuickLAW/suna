...
- `data:image/...;base64,...`：确认后保存到 `~/.suna/attachments/sha256-*.png`，再作为 attachment ref。
...
- 保留最近消息说明。
- 被压缩消息数和 summary token。
- 被截断工具输出数量。
- no-op 说明：当 working memory 不超过最近保留上限时，提示暂无可压缩内容。

交互原则：

- Compact 是模型上下文管理，不是 UI 历史删除。
- 手动 compact 成功后，agent working memory 会立即变为 summary + recent messages。
- TUI chat transcript 不替换、不删除旧消息，只追加 compact 结果面板。
- 面板必须说明“较早聊天仍保留在界面中；模型将基于摘要和最近消息继续”，避免用户误以为模型仍逐条持有所有旧消息。
- 自动 compact 不需要弹出独立交互，也不应该突然改写聊天窗口；错误只在 compact 后仍超安全阈值时通过常规错误流展示。
...
- Help 不展示高级 guard.blocked/allowed 规则细节；高级配置以 `plans/04-guard.md` 和 `~/.suna/config.toml` 为准。
