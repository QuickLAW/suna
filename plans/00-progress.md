...
| Spawn | Usable MVP | subtask 能跑，model/tools 必填且执行层校验，sub 只看到授权 tools，禁用 askuser/spawn；sub usage 计入 main session，tool call/result 经 main 事件流可见 | 补最终 model/tools 展示和 TUI 层级呈现 |
...
- subtask 继承全局 Guard policy、blocked/allowed、audit DB，并在 smart mode 下可使用同一 LLM reviewer。
- subtask 的 tool call/result 通过 main event stream 转发；stream/reasoning 不外显。
