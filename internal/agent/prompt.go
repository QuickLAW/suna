...
func (a *Agent) buildToolDefs() []model.ToolDef {
	...
	defs = append(defs, model.ToolDef{Name: "spawn", Description: "Run an isolated subtask with the selected model and only the specified tools. The subtask completes the task independently and returns its result; multimodal image input is supported.", Parameters: withIntentParameter(map[string]any{
		"type": "object", "properties": map[string]any{"task": map[string]any{"type": "string", "description": "Self-contained task for the subtask"}, "model": map[string]any{"type": "string", "description": "Exact model ref from Available subtask models"}, "system": map[string]any{"type": "string", "description": "Optional subtask system prompt"}, "tools": map[string]any{"type": "array", "items": map[string]any{"type": "string", "enum": spawnToolNames}, "description": "Allowed tools for the isolated subtask; use [] for model-only tasks"}, "input_images": map[string]any{"type": "array", "items": map[string]any{"type": "integer"}, "description": "Current user image indexes to pass to the subtask, e.g. [0] for the first image"}, "timeout": map[string]any{"type": "integer", "description": "Timeout seconds"}, "context": map[string]any{"type": "string", "description": "Extra context"}}, "required": []string{"task", "model", "tools"},
	})})
	return defs
}
