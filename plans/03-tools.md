...
```
功能: 结构化本地搜索，可替代常见 find/grep/glob/rg/global-style 搜索用途；支持路径、正文和结构入口搜索
参数: {
  path: string,                                           // 必填；文件或目录，优先使用项目相对路径
  pattern?: string,                                      // 单个搜索模式；默认 literal，适合普通文本、路径片段和带标点内容
  terms?: [string],                                      // 多个 literal 候选词；任意命中即可，优先用于多关键词搜索
  mode?: "auto"|"content"|"path"|"symbol",            // 默认 auto；content 类似 grep/rg，path 类似 find/fd/glob，symbol 搜结构入口
  match?: "literal"|"regex"|"glob",                    // 默认 literal；regex 仅用于有效 Go regexp，glob 主要用于 mode=path
  case?: "smart"|"insensitive"|"sensitive",            // 默认 smart；全小写时不敏感，含大写时敏感
  scope?: "workspace"|"deps"|"all",                    // 默认 workspace；deps 用于依赖源码，all 搜更大范围但仍保护敏感文件
  word?: bool,                                           // 完整标识符式词边界匹配；通常只在短词噪音多时使用
  include?: [string],                                    // glob include，例如 ["**/*.go", "docs/**"]
  exclude?: [string],                                    // 追加排除 glob
  context?: int,                                         // 默认 1，最大 5
  limit?: int,                                           // 默认 100，最大 1000
  depth?: int                                            // 默认 8，最大 20；0 表示只搜索当前目录
}
返回: 命中文本摘要；content/symbol 带上下文行，path 返回匹配路径；terms 命中会标注 matched term；内部标记 truncated 和 metadata

边界处理:
  - pattern 和 terms 至少提供一个；terms 永远按 literal alternatives 处理
  - 默认 match=literal，避免普通标点、路径、括号等被误当成正则
  - match=glob 支持常见 ** 语义，例如 **/*.go、internal/**/search*.go
  - 默认 scope=workspace，跳过依赖、构建产物、缓存、VCS 和常见敏感文件
  - scope=deps 会允许 node_modules/vendor 等依赖源码，但仍跳过构建/缓存/VCS/敏感文件
  - scope=all 会搜索生成物、缓存、依赖和 VCS 等更大范围，但仍保护常见敏感文件
  - 默认 depth=8，最大 20；默认 limit=100，最大 1000
  - 单文件超过 2MB 跳过；二进制文件跳过；总扫描文件数有内部上限
  - 空结果会给 diagnostics，例如提示 literal 中的 | 应改用 terms，或提示 scope/include/depth 可能过窄
```
...
所有工具返回的内容都有上限:
  ...
  Search:     默认 100 个匹配，最大 1000；输出 100KB；单文件扫描 2MB；最多扫描 20000 文件
...
超过上限:
  ...
  LLM 看到 truncated 后可以决定:
    ...
    - 用 search 的 pattern/terms、mode、include/exclude、scope、depth、limit 缩小范围
