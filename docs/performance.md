...
streaming 文本只追加时，TUI 会复用当前消息已经 wrap 好的行，并只增量处理新 delta、换行状态和最后一行宽度；不会在每个 chunk 上重新 split/wrap/join 完整已生成回复。窗口宽度变化或内容回退时才完整重算。这个优化不丢弃 delta、不延迟 done/tool/guard 等关键事件，而是用少量当前 streaming 消息的渲染行缓存换掉长回复 O(n²) 重排 CPU。缓存规模与当前消息长度同阶，不随 chunk 数增长。
