...
Guard 不再保存全局 `recentCtx`。每次 tool call 会由 runner/agent 构造不可变 `ReviewContext` 并传入 `Guard.Check()`，因此并发工具调用不会串上下文。subtask 的工具调用同样使用该机制。
...
- sub 的 Guard review 使用每次 tool call 的 `ReviewContext`（tool intent、assistant context、最近摘要）。
