...
func (a *Agent) buildToolDefs() []model.ToolDef {
	...
	defs = append(defs, model.ToolDef{Name: "askuser", Description: "Ask the user a question and wait for their reply", Parameters: withIntentParameter(map[string]any{
		"type": "object", "properties": map[string]any{"question": map[string]any{"type": "string", "description": "Question to ask"}, "options": map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Options"}}, "required": []string{"question"},
	})})
	spawnToolNames := a.availableSpawnTools()
	defs = append(defs, model.ToolDef{Name: "spawn", Description: "Create a subtask to execute a self-contained task. Choose model and tools explicitly. Tools are permissions, not preferences; use least privilege.", Parameters: withIntentParameter(map[string]any{
		"type": "object", "properties": map[string]any{"task": map[string]any{"type": "string", "description": "Subtask description"}, "model": map[string]any{"type": "string", "description": "Exact model ref from Available subtask models"}, "system": map[string]any{"type": "string", "description": "Subtask system prompt"}, "tools": map[string]any{"type": "array", "items": map[string]any{"type": "string", "enum": spawnToolNames}, "description": "Explicit tool permissions for the subtask; choose by least privilege"}, "timeout": map[string]any{"type": "integer", "description": "Timeout seconds"}, "context": map[string]any{"type": "string", "description": "Extra context"}}, "required": []string{"task", "model", "tools"},
	})})
	return defs
}
