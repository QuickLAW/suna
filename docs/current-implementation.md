...
reasoning 展开/折叠也只渲染主界面实际显示需要的小窗口：折叠态摘要从尾部小窗口提取最后一句，运行中 detail 渲染尾部有限行数/字节，完成态 detail 渲染头部有限行数/字节。已完成 reasoning 离屏时复用 line count；tab 会先展开为空格再 wrap，避免终端 tab stop 和宽度计算不一致导致边框错位。
